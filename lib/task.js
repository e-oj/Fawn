...
var Task = function(){
  ...
  task.initModel = function(modelName, schema){
    ...
    if(modelCache[modelName]) throw new Error("Invalid model");
    if(!isObject(schema)) throw new Error("Invalid Schema");

    return getModel(modelName, schema ? new Schema(schema, {strict: true}) : null);
  };

  //TODO: Add options arg to update function
...
var performUpdate = function(update, task) {
  ...
  return Collection.find(update.condition)
    .exec()
    .then(function(docs){
      var oldData = [];

      for(var i = 0; i < docs.length; i++) {
        oldData.push(docs[i]._doc);
      }
      ...
      return updateState(task, update.index, PENDING)
        .then(function(_task){
          return Collection.update(update.condition, update.data, {multi: true})
            .exec()
            .then(function(){
