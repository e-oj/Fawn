...
var performSave = function(save, task){
  return new Promise(function(resolve, reject){
    ...
    try {
      Collection = newCollection(save.name, defaultSchema);
    } catch(err){
      if(err.name = "OverwriteModelError"){
        Collection = getCollection(save.name);
      }
      else throw err;
    }
...
var getCollection = function(name){
  return mongoose.model("blah", defaultSchema, name);
};

var newCollection = function(name, schema){
